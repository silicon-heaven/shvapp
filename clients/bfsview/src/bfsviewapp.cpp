...
static std::vector<cp::MetaMethod> meta_methods_pwrstatus {
	cp::methods::DIR,
	cp::methods::LS,
	{cp::Rpc::METH_GET, cp::MetaMethod::Flag::IsGetter, {}, "UInt", cp::AccessLevel::Browse, {{cp::Rpc::SIG_VAL_CHANGED}}},
	{cp::Rpc::METH_SET, cp::MetaMethod::Flag::None, "UInt", "Bool", cp::AccessLevel::Write},
...
{
	std::string shv_path = shv::core::utils::ShvPath(bfsPlcShvPath()).appendDir(std::string("status")).asString();
	return shv_path;
}
