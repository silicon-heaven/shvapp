	...
	, ui(new Ui::MainWindow)
{
	...
	connect(ui->oclOn, &QCheckBox::toggled, [](bool on) {
		...
		s = Ams005YcpApp::setMask(s, Ams005YcpApp::ConvStatus::OCL_VOLTAGE, on);
		app->setConvStatus(s);
	});
	connect(ui->doorOpen, &QCheckBox::toggled, [](bool on) {
		Ams005YcpApp *app = Ams005YcpApp::instance();
		unsigned s = app->convStatus();
		s = Ams005YcpApp::setMask(s, Ams005YcpApp::ConvStatus::DOOR_OPEN, on);
		app->setConvStatus(s);
	});
	connect(ui->acReady, &QCheckBox::toggled, [](bool on) {
		Ams005YcpApp *app = Ams005YcpApp::instance();
		unsigned s = app->convStatus();
		s = Ams005YcpApp::setMask(s, Ams005YcpApp::ConvStatus::AC_READY, on);
		app->setConvStatus(s);
	});
