...
int main(int argc, char *argv[])
{
	...
	if(cli_opts.isAppBreak()) {
		if(cli_opts.isHelp()) {
			...
			ts << "--use-syslog\n";
			ts << "\tsend log to syslog instead of stderr\n";
		}
		return EXIT_SUCCESS;
	}
	...
	//shvInfo()<< "Logging treshold:" << shv::core::ShvLog::levelToString(shv::core::ShvLog::globalLogTreshold());
	//shvInfo().noquote() << "Logged modules:" << qf::core::LogDevice::modulesLogInfo();
	//shvInfo().noquote() << "Logged categories:" << qf::core::LogDevice::categoriesLogInfo();
	shvInfo() << "--------------------------------------------------------------------------------------";
	//break;
