...
PROJ_NAME=shv
SRC_DIR=/home/fanda/proj/$PROJ_NAME
WORK_DIR=/home/fanda/t/_distro
DISTRO_NAME=$PROJ_NAME-linux64-$DISTRO_VER
...
cd $BUILD_DIR
$QMAKE $SRC_DIR/$PROJ_NAME.pro CONFIG+=release CONFIG+=force_debug_info CONFIG+=separate_debug_info -r -spec linux-g++
make -j2
if [ $? -ne 0 ]; then
  echo "Make Error" >&2
  exit 1
