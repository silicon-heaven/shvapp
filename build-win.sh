...
c:/Qt/5.12.0/mingw73_64/bin/qmake.exe DEFINES+=GIT_COMMIT=${CI_COMMIT_SHA} DEFINES+=GIT_BRANCH=${CI_COMMIT_REF_SLUG} MINGW_DIR=c:/Qt/Tools/mingw730_64 -r CONFIG+=release CONFIG+=force_debug_info CONFIG+=separate_debug_info shv.pro || exit 2
c:/Qt/Tools/mingw730_64/bin/mingw32-make.exe -j4 || exit 2

#[ -d bin/translations ] || mkdir bin/translations  || exit 2

#VERSION=`grep BRCLAB_VERSION brclab/src/version.h | cut -d\" -f2`
