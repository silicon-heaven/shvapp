...
static std::vector<cp::MetaMethod> meta_methods {
	...
	{M_APP_VERSION, cp::MetaMethod::Flag::IsGetter, {}, "RpcValue", cp::AccessLevel::Browse},
	{M_GIT_COMMIT, cp::MetaMethod::Flag::IsGetter, {}, "RpcValue", cp::AccessLevel::Read},
	{M_SHV_VERSION, cp::MetaMethod::Flag::IsGetter, {}, "RpcValue", cp::AccessLevel::Read},
	{M_SHV_GIT_COMMIT, cp::MetaMethod::Flag::IsGetter, {}, "RpcValue", cp::AccessLevel::Read},
	...
	//{cp::Rpc::METH_HELP, cp::MetaMethod::Signature::RetParam, 0, cp::AccessLevel::BROWSE},
	{cp::Rpc::METH_RUN_CMD, cp::MetaMethod::Flag::None, "String|List", "Map|Int|String|List", cp::AccessLevel::Command, {},
	...
	{cp::Rpc::METH_RUN_SCRIPT, cp::MetaMethod::Flag::None, "String|List", "Map|Int|String|List", cp::AccessLevel::Command, {},
		"Parameter can be string or list\n"
	...
	{cp::Rpc::METH_LAUNCH_REXEC, cp::MetaMethod::Flag::None, "RpcValue", "RpcValue", cp::AccessLevel::Command},
};
...
shv::chainpack::RpcValue AppRootNode::callMethodRq(const shv::chainpack::RpcRequest &rq)
{
	if(rq.shvPath().asString().empty()) {
		...
		if(rq.method() == cp::Rpc::METH_RUN_SCRIPT) {
			...
			if(rq.params().isList()) {
				const auto params = rq.params();
				for(auto &p : params.asList()) {
					if(p.isString())
